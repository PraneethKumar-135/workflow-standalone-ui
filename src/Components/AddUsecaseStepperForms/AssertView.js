...
const AssetView = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://m41stqhs8f.execute-api.us-east-1.amazonaws.com/dev/usecase/${addUsecaseId}/asset`);
                                ...
                                {data[key].documents.map((document, idx) => (
                                    <React.Fragment>
                                        <div key={idx} className="bg-[#E6F7FF] w-full h-12 mb-2 flex justify-between items-center px-4 rounded-md">
                                            <h2 className='text-xl font-semibold'>{document.docName}</h2>
